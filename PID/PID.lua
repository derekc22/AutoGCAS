...
dataref("pitchRate", "sim/flightmodel/position/Q", "writeable") -- Degrees/second, The pitch rotation rates (relative to the flight) (-ve down, +ve up)
dataref("rollRate", "sim/flightmodel/position/P", "writeable")    -- Degrees/second, The roll rotation rates (relative to the flight) (-ve left, +ve right)
dataref("yawRate", "sim/flightmodel/position/R", "writeable")  -- Degrees/second, The yaw rotation rates (relative to the flight) (-ve left, +ve right)
...
-- The aircraft's velocities will only change if 'override_planepath' == 0
...
local function read_input_file(path)
    ...
    for line in allLines do
        if tonumber(line) == nil then line = 0 end
...
local function write_data(path, dataToWrite)
    ...
    for index, data in ipairs(dataToWrite) do
        ...
        outputDataString = outputDataString .. delimeter .. data
    end
...
function tune_PID()
    ...
    -- PERFORM TESTS WITH THROTTLE = 1 to simulate behavior of control surfaces during realistic stall event (where throttle would be set to max)
...
function tune_throttle_PID()

    local throttleFixPoint = 0.5
    -- PERFORM TESTS GOING FROM 'NORMAL' THROTTLE SETTING (as specified by 'throttleFixPoint') TO MAX THROTTLE SETTING to simulate pliot behavior before and during a realistic stall event (where aircraft rapdily goes from 'normal' power setting to max throttle setting)
    ...
    if throttle == throttleFixPoint then
        initialChangeDetected = 0
    else
...
function test_PID()
    ...
    if elapsedTime < 20 then
        ...
        vz = -150/ktsToMetersPerSec -- kts to m/s
        y = fixedAltitudeFt/ftToMeters -- ft to m
    end
    ...
    write_data(outputDataPath, {elapsedTime, rightAileron, roll})
    update_aircraft()
